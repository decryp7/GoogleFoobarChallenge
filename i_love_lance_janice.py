...
you're pretty sure it's not job-related -- they're both huge fans of the space soap opera ""Lance & Janice"". You
know how much Commander Lambda hates waste, so if you can prove that these minions are wasting time passing
...
punctuation) is left untouched.  That is, 'a' becomes 'z', 'b' becomes 'y', 'c' becomes 'x', etc.  For instance,
...
def solution(x):
    ...
    for c in x:
        if c.isalnum():
            try:
                index_of_code = codes.index(c)
                result += keys[index_of_code]
            except ValueError:
                result += c
        else:
            result += c
